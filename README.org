...
Component =gt-taker=, =gt-engine= and =gt-render= are inherited from =gt-validator=, which
...
Symbol can be prefixed with =not-= to indicate a reverse determination.
...
- =not-word= reverse determination, translated text *is not* word
